...
impl Md5 {
    fn transform_blocks(&mut self) {
        ...
        for (index, index2) in [0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60]
            .iter()
            .enumerate()
        {
            let index2_u = *index2 as usize;
            x[index] = (self.block[index2_u] as u32)
                | ((self.block[index2_u + 1] as u32) << 8)
                | ((self.block[index2_u + 2] as u32) << 16)
                | ((self.block[index2_u + 3] as u32) << 24);
        }
    ...
    fn transform(&mut self, block: &[u8]) {
        ...
        {
            let index2_u = *index2 as usize;
            x[index] = (block[index2_u] as u32)
                | ((block[index2_u + 1] as u32) << 8)
                | ((block[index2_u + 2] as u32) << 16)
                | ((block[index2_u + 3] as u32) << 24);
        }
        ...
        if data_len >= partial_len_usize {
            ...
            unsafe {
                core::intrinsics::copy_nonoverlapping(
                    data.as_ptr(),
                    self.block[(index_usize)..].as_mut_ptr(),
                    partial_len_usize,
                );
            }
